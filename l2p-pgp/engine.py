            ...
            for k, (m, params) in enumerate(model.named_parameters()):
                if m == "prompt.prompt":
                    params.grad.data[prompt_idx] = params.grad.data[prompt_idx] - torch.matmul(
                        params.grad.data[prompt_idx], feature_mat)  # 梯度投影
...
def train_and_evaluate(model: torch.nn.Module, model_without_ddp: torch.nn.Module, original_model: torch.nn.Module, 
    ...
    for task_id in range(args.num_tasks):
        if not args.no_pgp:
            model.eval()
            original_model.eval()
            mem_example = memory.get_representation_matrix(data_loader[task_id]['mem'], device)  # [768,3,224,224]
            rep, rep_key = memory.get_rep(model, original_model, mem_example, task_id) # 取采样样本的embedding

            # image embedding
            rep = torch.cat(rep)  # [768,196 * 768] = [768,150528]
            rep = rep.detach().cpu().numpy()
            pca = PCA(n_components=9)
            pca = pca.fit(rep)
            rep = pca.transform(rep)  # [768,9]

            if task_id != 0:
                for k, (m, params) in enumerate(model.named_parameters()):
                    if m == "prompt.prompt":
                        p_ = params.data  # [10,5,768]
                        p_ = p_.view(-1, 768).detach().cpu().numpy().transpose(1, 0)  # [768,50]
                # 构建pca
                pca = PCA(n_components=9)
                pca = pca.fit(p_)
                p = pca.transform(p_)  # [768,9]
            ...
            pca = pca.fit(rep_key)
            rep_key = pca.transform(rep_key)  # [768,5]
            ...
            feature = memory.update_memory(rep, 0.50, feature) #  对embedding进行svd，得到投影矩阵
            key_feature = memory.update_memory(rep_key, 0.97, key_feature)  # key的投影矩阵
