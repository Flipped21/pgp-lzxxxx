...
class TPrompts(BaseLearner):
    ...
    def _train(self, train_loader, test_loader):
        ...
        self._network.eval()
        mem_example, mem_tasks = memory.get_representation_matrix(train_loader, self._device)  # [768,3,224,224]
        rep = []
        with torch.no_grad():
            for bs_ in range(24):
                rep_ = self._network.query(mem_example[bs_ * 32:(bs_ + 1) * 32, ...], mem_tasks[bs_ * 32:(bs_ + 1) * 32])  # 一个batch的特征 [32, 197, 768]
                # rep_ = rep_.reshape(32, -1)  #[32,197×768]=[32,151296]
                rep_ = rep_.permute(2, 0, 1)  # [768, 32, 197]
                rep.append(rep_)
        # rep = torch.cat(rep)  #  [768, 151296]
        rep = torch.cat(rep, dim=1)  #  [768,32*24,197]
        rep = rep.reshape(768, -1)  # [768,151296]

        rep = rep.detach().cpu().numpy()
        pca = PCA(n_components=5)
        pca = pca.fit(rep)
        rep = pca.transform(rep)  # [768,5]
        self._network.train()

        # Prompt Gradient Projection Matrix
        for k, (m, params) in enumerate(self._network.named_parameters()):
            if "image_prompt_pool" in m:
                p_ = params.data  # [10,768]
                p_ = p_.view(-1, 768).detach().cpu().numpy().transpose(1, 0)  # [768,10]
                pca = PCA(n_components=5)
                pca = pca.fit(p_)
                p_ = pca.transform(p_)  #[768,5]
