...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, instance_tokens=None):
        ...
        self.act['rep'] = x  # [32,197,768]

        if instance_tokens is not None:
            x = torch.cat([x[:,:1,:], instance_tokens, x[:,1:,:]], dim=1)   # 嵌入图像prompt  [32,207,768]

        x = self.ln_pre(x)
        x = x.permute(1, 0, 2)  # NLD -> LND  [207,32,768]
        x = self.transformer(x)  # [207,32,768]
        x = x.permute(1, 0, 2)  # LND -> NLD [32,207,768]

        x = self.ln_post(x[:, 0, :]) # 只保留CLS token的内容，其他舍弃  [32,768]

        if self.proj is not None:
            x = x @ self.proj  # [32,512]
