...
class TextEncoder(nn.Module):
    def __init__(self, clip_model):
        super().__init__()
        self.transformer = clip_model.transformer  # 看model.py中的build_model()
    ...
    def forward(self, prompts, tokenized_prompts):
        x = prompts + self.positional_embedding.type(self.dtype)  # Embedding Prompts [10,77,512]
        x = x.permute(1, 0, 2)  # [77,10,512]
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # [10,77,512]
        x = self.ln_final(x).type(self.dtype)
        x = x[torch.arange(x.shape[0]), tokenized_prompts.argmax(dim=-1)] @ self.text_projection  # [10,512]
        return x
    ...
    def __init__(self, cfg, classnames, clip_model):
        ...
        if ctx_init:
            ...
            if cfg.CSC:
                ...
                ctx_vectors = torch.empty(n_cls, n_ctx, ctx_dim, dtype=dtype)   # [10,16,512]上下文向量，可以是类别特定的或通用的，取决于配置参数。
            else:
