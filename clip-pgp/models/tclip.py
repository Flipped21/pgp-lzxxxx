...
class Ticlip(nn.Module):
    ...
    def query(self, images, tasks):
        tasks = tasks.cpu().tolist()
        selects = [0 for i in range(len(tasks))]
        image_prompts = torch.stack([prompt.weight for prompt in self.image_prompt_pool], 0)[selects, :, :]  # [32,10,768]相当于32个一样的图像提示
        _ = self.image_encoder(images.type(self.dtype), image_prompts)  #图像和提示一起输入encoder [32,512]
