...
from datasets.data import iCifar100, iImageNetR
    ...
    def get_dataset(self, indices, mode, appendent=None, ret_data=False):
        ...
        if self.dataset_name == "cifar100":
            ...
            if mode == "train":
                ...
                data_path = os.path.join(self.args["data_path"], "test")
            else:
                raise ValueError("Unknown mode {}.".format(mode))
            
            # 使用ImageFolder加载ImageNet-R
    ...
    def _select(self, dataset, low_range, high_range):
        ...
        if self.dataset_name == "cifar100":
            ...
            for idx in range(len(dataset)):
                image_path, target = dataset.samples[idx]
                target = int(target)  # 确保target是整数
                if high_range > target >= low_range:
                    # 直接打开图像文件
...
class DummyDataset(Dataset):
    def __init__(self, images, labels, transforms=None):
        ...
        # 根据标签数量动态计算tasks
        self.tasks = labels // (len(set(labels)) // 10) if len(set(labels)) > 0 else labels
    ...
    def __getitem__(self, item):
        ...
        if self.trans:
            ...
            img = self.trans(img)
        return self.tasks[item], img, self.labels[item]
